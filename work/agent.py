...
class WorkAgent:
    ...
    def execute_plan(self, plan: Dict) -> Dict[str, Any]:
        ...
        for i, step in enumerate(steps):
            if last_result_path and step.get("type") in ["elevation", "slope"]:
    ...
    def _extract_action(self, thought: str) -> Dict:
        import re
        ...
        tool_name = None
        if "buffer_filter" in thought.lower() or "缓冲区" in thought:
            tool_name = "buffer_filter_tool"
        elif "elevation_filter" in thought.lower() or "高程" in thought:
            tool_name = "elevation_filter_tool"
        elif "slope_filter" in thought.lower() or "坡度" in thought:
            tool_name = "slope_filter_tool"
